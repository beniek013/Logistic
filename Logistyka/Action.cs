...
namespace Logistyka
{
    class Action
    {
        ...
        private int eet;
        private int let;
        private Action[] successors;
        private Action[] predecessors;
        ...
        public Action CheckActivity(Action[] list, string id, int i)
        {
            for (int j = 0; j < i; j++)
            {
                if (list[j].Id == id)
        ...
        public int GetIndex(Action[] list, Action aux, int i)
        {
            for (int j = 1; j < i; j++)
            {
                if (list[j].Id == aux.Id)
        ...
        /// <returns>aux</returns>
        public Action SetSuccessors(Action aux, Action activity)
        {
            if (aux.Successors != null)
            {
                Action aux2 = new Action();
                aux2.Successors = new Action[aux.Successors.Length + 1];
